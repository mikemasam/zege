use crate::{api::reports::ZegeReport, ctx::appcontext::AppContext, utils::{dbutil::rows_to_json_vec, http::AppResponse}};
use axum::{
...
pub async fn report_read_route(
    ...
    let results = rows_to_json_vec(rows).await;
    AppResponse::ok(results.ok(), None)
}
