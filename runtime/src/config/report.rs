...
pub async fn report_create_route(
    ...
    let mut query = QueryBuilder::<sqlx::Sqlite>::new(
        " insert into zg_reports (report_name, report_type, report_sql) ",
    );

    query
        .push_bind(item.report_name)
        .push_bind(item.report_type)
        .push_bind(item.report_sql);

    let app = appcontext.lock().await;
    let configdb = app.configdb.as_ref().unwrap();
    let _db = configdb.lock().await;
    let res = query.build().execute(_db.pool.as_ref().unwrap()).await.unwrap();
    AppResponse::<Value> {
        status: 200,
        message: String::new(),
        data: serde_json::to_value(res.rows_affected()).ok(),
    }
}
