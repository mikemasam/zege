...
use crate::{ctx::{appcontext::DbStorage, dbmanager::DatabasePool}, utils::{appenv::AppLogger, security::Security}};
...
impl User {
    pub async fn list(db: DbStorage) -> Result<Vec<User>> {
    ...
    pub async fn find_by_email(storage: DbStorage, email: String) -> Result<User> {
    ...
    pub async fn find_by_id(db: DbStorage, id: i64) -> Result<User> {
        let pool = db.pool.clone();
        match pool.as_ref().unwrap() {
            DatabasePool::Postgres(pool) => {
    ...
    pub async fn create(db: DbStorage, item: NewUser) -> Result<User> {
        let pool = db.pool.clone();
